...
<nav class="navbar navbar-expand navbar-light bg-white topbar mb-4 static-top shadow">
    ...
    <h5 class="d-none d-sm-inline-block m-0 text-gray-800 font-weight-bold">Passengers</h5>
...
<div class="d-sm-flex align-items-center justify-content-between mb-4">
    <h1 class="h3 mb-0 text-gray-800">Passenger List</h1>
    <a href="passenger_form.php" class="d-none d-sm-inline-block btn btn-sm btn-primary shadow-sm">
        <i class="fas fa-user-plus fa-sm text-white-50"></i> Register Passenger
    </a>
</div>
            ...
            <table class="table table-bordered table-hover data-table" width="100%">
                ...
                <?php if (!empty($passengers)): ?>
                    <?php foreach ($passengers as $p): ?>
                        <tr>
                        <td><?php echo htmlspecialchars($p["passport_no"] ?? "", ENT_QUOTES, "UTF-8"); ?></td>
                        <td><?php
                            $fullName = trim(($p["first_name"] ?? "") . " " . ($p["middle_name"] ?? "") . " " . ($p["last_name"] ?? ""));
                            echo htmlspecialchars($fullName, ENT_QUOTES, "UTF-8");
                        ?></td>
                        <td><?php echo htmlspecialchars($p["date_of_birth"] ?? "", ENT_QUOTES, "UTF-8"); ?></td>
                        <td><?php echo htmlspecialchars($p["gender"] ?? "", ENT_QUOTES, "UTF-8"); ?></td>
                        <td><?php echo htmlspecialchars($p["email"] ?? "", ENT_QUOTES, "UTF-8"); ?></td>
                        <td><?php echo htmlspecialchars($p["contact_number"] ?? "", ENT_QUOTES, "UTF-8"); ?></td>
                        <td><?php echo htmlspecialchars($p["nationality"] ?? "", ENT_QUOTES, "UTF-8"); ?></td>
                        <td><span class="badge badge-<?php echo htmlspecialchars($p["status"] ?? "", ENT_QUOTES, "UTF-8"); ?>"><?php echo htmlspecialchars($p["status"] ?? "", ENT_QUOTES, "UTF-8"); ?></span></td>
                        <td>
                            <a href="passenger_form.php?id=<?php echo (int) ($p['passenger_id'] ?? 0); ?>" class="btn btn-sm btn-warning" title="Edit"><i class="fas fa-edit"></i></a>
                            <button type="button" class="btn btn-sm btn-danger" title="Delete"
                                    data-toggle="modal" data-target="#deleteModal<?php echo (int) ($p['passenger_id'] ?? 0); ?>">
